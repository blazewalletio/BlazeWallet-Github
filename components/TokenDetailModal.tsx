...
export default function TokenDetailModal({
  ...
  const { currentChain, getCurrentAddress, hideToken, isTokenHidden, deleteToken, isTokenDeleted } = useWalletStore();
                    ...
                    <div className="space-y-3 pt-4 mt-4 border-t border-gray-200">
                      ...
                      <div className="pt-3 mt-3 border-t border-gray-200 space-y-2">
                        ...
                        <motion.button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (!token.address || isNative) return;
                            setShowDeleteConfirm((prev) => !prev);
                          }}
                          disabled={!token.address || isNative || isTokenDeleted(currentChain, token.address || '')}
                          className="flex items-center gap-3 w-full p-3 rounded-lg hover:bg-red-50 transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <Trash2 className="w-5 h-5 text-red-600" />
                          <div>
                            <div className="font-medium text-gray-900 text-sm">
                              {isNative
                                ? 'Cannot delete native token'
                                : !token.address
                                  ? 'Cannot delete token'
                                  : isTokenDeleted(currentChain, token.address)
                                    ? 'Token deleted'
                                    : 'Delete token'}
                            </div>
                            <div className="text-xs text-gray-500">Permanently remove from assets</div>
                          </div>
                        </motion.button>

                        {showDeleteConfirm && token.address && !isNative && (
                          <div className="rounded-lg border border-red-200 bg-red-50 p-3">
                            <p className="text-xs text-red-800 mb-3">
                              Delete <span className="font-semibold">{token.symbol}</span> from your assets list? You can add it again later by searching or entering the address.
                            </p>
                            <div className="flex gap-2">
                              <button
                                onClick={() => setShowDeleteConfirm(false)}
                                className="flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                              >
                                Cancel
                              </button>
                              <button
                                onClick={() => {
                                  deleteToken(currentChain, token.address);
                                  logger.log(`🗑️ Token ${token.symbol} deleted on chain ${currentChain}`);
                                  onClose();
                                }}
